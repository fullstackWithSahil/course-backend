...
export default async function Transcode(resolution: "1080" | "720" | "360" | "144") {
    const scale = getRatio(resolution);
    try {

        // Step 1: Transcode video
        await execPromise(`ffmpeg -i downloads/input.mp4 -c:v libx264 -c:a aac -vf scale=${scale} -f mp4 output/i${resolution}.mp4`);
